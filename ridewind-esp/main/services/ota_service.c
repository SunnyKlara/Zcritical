...
/* Helper: notify both BLE and WebSocket channels.
 * OTA commands can arrive from either channel, so responses go to both. */
static void ota_notify_str(const char *str)
{
...
bool ota_service_begin(uint32_t firmware_size, const char *sha256_hex)
{
    ...
    if (!s_ota.partition) {
        ...
        ota_notify_str("OTA_FAIL:NO_PARTITION\r\n");
        return false;
    }
    ...
    while (left > 0) {
        ...
        if (s_ota.buf_offset >= OTA_BUF_SIZE) {
            if (!flush_buffer()) {
                ...
                ota_notify_str("OTA_FAIL:FLASH_WRITE\r\n");
                ota_service_abort();
                return;
            }
        }
...
{
    if (!s_ota.active) {
        ota_notify_str("OTA_FAIL:NO_SESSION\r\n");
        return;
    }
    ...
    if (s_ota.sha_active) {
        ...
        if (strcmp(calc_hex, s_ota.expected_sha256) != 0) {
            ...
            ota_notify_str("OTA_FAIL:SHA256\r\n");
