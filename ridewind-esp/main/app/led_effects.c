...
/* ── Effect 5: Wave (Continuous sine wave — left to right) ──
 *
 * A sine brightness wave continuously rolls from left to right across
 * the 6 Main LEDs. No gaps, no stops — endless like ocean waves.
 * Base 30%, peak 100%. One full wave cycle = 2.5s.
 * The wave wraps around seamlessly. */
static void throttle_fx_wave(uint8_t spd)
{
    ...
    for (int i = 0; i < LED_TAIL_COUNT; i++) {
        uint8_t led_phase = wave_phase + (uint8_t)((2 - i) * 42) - 60;
