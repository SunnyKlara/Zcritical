...
class _DeviceConnectScreenState extends State<DeviceConnectScreen>
  ...
  void _showDeviceMenu(BuildContext context) {
    device_dialogs.showDeviceMenu(
      context,
      onLogoUpload: () {
        if (mounted) _showLogoUploadScreen(context);
      },
      onOtaUpgrade: () {
        if (mounted) _navigateToOtaUpgrade(context);
      },
      onWifiProvisioning: () {
        if (mounted) _showWifiProvisioningDialog(context);
      },
      onRemoveDevice: () {
        if (mounted) _showRemoveDeviceDialog(context);
      },
    );
  }
  ...
  void _showLogoUploadScreen(BuildContext parentContext) {
    ...
    device_dialogs.navigateToLogoUpload(
      context,
      btProvider: btProvider,
      lastSentHardwareUI: _lastSentHardwareUI,
      currentModeIndex: _currentModeIndex,
      onHardwareUIChanged: (val) {
        if (mounted) setState(() => _lastSentHardwareUI = val);
      },
    );
  }
