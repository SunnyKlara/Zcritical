...
- ✅ **固件 CI 自动化上线**（commit `dcd3d50`）— `fw-v*` tag 触发 ESP-IDF Docker 构建 + 自动创建 Release + 上传 .bin + jq 回写 firmware.json
- ✅ git-and-release.md 文档同步：固件发版从 8 步手工简化为 3 步
...
**下一步行动（按顺序）**：
1. **先确诊 CI 不触发的根因**（用户在浏览器查 Actions 设置 + Actions 列表）
2. **决定 LFS 修复方案**（推荐 B：wav 移出 LFS）
3. **fw-v1.2.0 补全**（CI 修好后会自动跑；不修就手工上传 .bin）
4. 真机验证 v1.3.0 用户可见功能
5. 真机调试 DEBUG_PLAN 第 1-2 轮（设备重启根因定位）
6. iOS 上架准备
7. iOS 上架准备
