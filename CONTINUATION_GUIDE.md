...
| 状态 | 问题 | verified |
|------|------|----------|
| 🚨 P0 阻塞 | **v1.2.1 APP 升级失败** — tag 命名 `v1.2.1` 不匹配 workflow 触发条件 `app-v*`，CI 未运行，APK 未上传到 GitHub Releases。同时 `app_version.json` 的 download_url 指向私有服务器（47.107.143.4）也无文件。修复方案：`git tag app-v1.2.1 v1.2.1 && git push origin app-v1.2.1` 触发 CI，或手动构建上传 | 2026-05-24 |
...
**产品化 P0 任务（下一步立即执行）**：
1. 品牌重命名 + 包名修改（Kotlin 包路径、gradle、pubspec、APP 内文字）
2. 资源瘦身（car_thumbnails 88MB + engine_individual 299MB 移出 APK，改按需下载）
3. Release 签名配置（生成正式 keystore）
4. 服务器加 HTTPS
