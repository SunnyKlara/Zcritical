...
**v7 性能优化**（解决极度卡顿）：
- 删除逐像素边框绘制（上千次单像素 SPI → 边框纳入 LUT 批量渲染）
- 刻度线从 21 条减到 5 条大刻度（3px 宽），删除刻度数字
- 指针重绘阈值（角度变化 < 0.02 rad 时跳过）
- 挡位方块改为递增宽度（4→18px）+ 渐变色（蓝→青→黄→橙→红）
- 编译状态：⚠️ 待 `.\build.ps1 -Full` 验证
...
| 🔴 阻塞 | BLE 断开弹窗仍频繁出现 — 已将静默等待从 15s 加强到 30s×3 次（总 90s），只有彻底连不上才弹窗。需确认用户编译的 APP 是否来自当前分支（feat/screen-refactor） | 2026-05-24 |
