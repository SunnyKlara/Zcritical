...
- 2026-05-17（第十六轮）：跑步机 UI v2→v3 迭代
  - v2：F4 贴图数字 + 4px 薄弧 + 小圆点指示器（极简但不符合产品风格）
  - v3 最终方案：完全镜像 Speed UI 设计
    - 背景：`gImage_beijing_240_240`（和 Speed 一样）
    - 数字：F4 大号白色贴图，右对齐 `(F4_X_QI, F4_Y_QI)`，jianju=-2，整数 0-20
    - 色条：数字下方 3px 高白色条，宽度跟随速度百分比
    - 操作：纯油门模式 — 按住加速(150ms/步)，松开减速(100ms/步)
    - 退出：双击或长按返回菜单
    - 无弧线、无文字、无小数点、无颜色变化
  - 设计决策：不模仿 Forza 弧形仪表盘，而是直接复用 Speed 界面的视觉语言
  - 分支：`fw/treadmill-panel`，commit `c301ded`
  - 待 idf.py build 验证编译
  - 后续：菜单界面图标需要单独设计（不再模仿 Speed）
