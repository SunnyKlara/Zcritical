...
## 教训 8：AI 不能基于文档描述做判断，必须读代码验证

**问题**：CONTINUATION_GUIDE 写"风扇无法调速（硬件限制）"，AI 直接当事实引用给建议，没有去读 `pin_config.h` 和 `drv_pwm.c` 验证实际状态
**结论**：任何涉及"硬件状态"、"是否已实现"、"是否有 bug"的判断，必须读对应代码文件确认，不能只信文档描述。文档可能过时，代码是真值源。
**违反后果**：给出错误建议，浪费用户时间，损害信任

## 教训 9：文档信息必须有单一真值源，不能多处重复

**问题**：git 规范同时写在 `git-workflow.md` 和 `git-and-release.md`，两个文件 commit 格式不同（一个中文一个英文），新 AI 不知道听谁的
**结论**：每类信息只在一个文件中定义。其他地方需要引用时用链接，不要复制内容。
**违反后果**：改了一处忘了另一处 → 文档冲突 → AI 行为不一致
